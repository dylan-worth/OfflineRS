   ...
   void method4046(RSByteBuffer buffer, int opcode, byte var3) {
      ...
      if(opcode == 1) {
         ...
         for(index = 0; index < count; ++index) {
            this.models[index] = buffer.readBigSmart();
         }
      } else if(2 == opcode) {
         ...
         for(index = 0; index < count; ++index) {
            this.originalColours[index] = (short)buffer.readUnsignedShort();
            this.replacementColours[index] = (short)buffer.readUnsignedShort();
         }
      } else if(opcode == 41) {
         ...
         for(index = 0; index < count; ++index) {
            this.originalTextures[index] = (short)buffer.readUnsignedShort();
            this.replacementTextures[index] = (short)buffer.readUnsignedShort();
         }
      } else if(opcode == 42) {
         ...
         for(index = 0; index < count; ++index) {
            this.recolourPalette[index] = buffer.readByte();
         }
      } else {
         int slot;
         int var7;
         ...
         if(44 == opcode) {
            ...
            for(index = 0; index < count; ++index) {
               this.headModels[index] = buffer.readBigSmart();
            }
         } else if(opcode == 93) {
            this.drawMapdot = false;
         } else if(opcode == 95) {
            this.combatLevel = buffer.readUnsignedShort() * -717243737;
         } else if(97 == opcode) {
            this.scaleX = buffer.readUnsignedShort() * 646993937;
         } else if(opcode == 98) {
            this.scaleY = buffer.readUnsignedShort() * -62586753;
         } else if(opcode == 99) {
            this.aBool3329 = true;
         } else if(100 == opcode) {
            this.ambience = buffer.readByte() * -167665889;
         } else if(opcode == 101) {
            this.anInt3338 = buffer.readByte() * 39459101;
         } else if(102 == opcode) {
            ...
            for(var7 = 0; var7 < index; ++var7) {
               if((count & 1 << var7) == 0) {
                  ...
                  this.headIconSprites[var7] = buffer.readBigSmart();
                  this.headIconSubSprites[var7] = (short)buffer.readDecoratedSmart();
               }
            }
         } else if(103 == opcode) {
            this.contrast = buffer.readUnsignedShort() * 1966581567;
            ...
            if(opcode == 107) {
               ...
               this.aShort3353 = (short)buffer.readUnsignedShort();
               this.aShort3345 = (short)buffer.readUnsignedShort();
            } else if(114 == opcode) {
               this.aByte3299 = buffer.readByte();
               this.aByte3347 = buffer.readByte();
            } else if(119 == opcode) {
               this.movementCapabilities = buffer.readByte();
            } else if(opcode == 121) {
               ...
               this.height = buffer.readUnsignedShort() * 722249395;
            } else if(125 == opcode) {
               this.respawnDirection = (NPCDirection)Class561.findIdentifiable(Class592.valuesOf(-1763260781), buffer.readByte(), (byte)-103);
            } else if(opcode == 127) {
               this.renderEmote = buffer.readUnsignedShort() * 1457391483;
            } else if(128 == opcode) {
               Class561.findIdentifiable(MovementDirection.method8035(2100716577), buffer.readUnsignedByte(1770117778), (byte)4);
            } else if(opcode == 134) {
               this.anInt3313 = buffer.readUnsignedShort() * 812768201;
               if(-1515639175 * this.anInt3313 == '\uffff') {
                  this.anInt3313 = -812768201;
               }

               this.anInt3354 = buffer.readUnsignedShort() * 2108060057;
               if('\uffff' == -803450199 * this.anInt3354) {
                  this.anInt3354 = -2108060057;
               }

               this.anInt3355 = buffer.readUnsignedShort() * 191791459;
               if(this.anInt3355 * 2040989771 == '\uffff') {
                  this.anInt3355 = -191791459;
               }

               this.anInt3356 = buffer.readUnsignedShort() * -1437088833;
               ...
               if(opcode == 137) {
                  this.attackCursor = buffer.readUnsignedShort() * 1637677659;
               } else if(opcode == 138) {
                  this.armyIcon = buffer.readBigSmart() * -230266399;
               } else if(opcode == 140) {
                  this.anInt3359 = buffer.readUnsignedByte(148452503) * -1337544833;
               } else if(141 == opcode) {
                  this.aBool3331 = true;
               } else if(opcode == 142) {
                  this.mapFunction = buffer.readUnsignedShort() * 1821656325;
               } else if(opcode == 143) {
                  this.aBool3330 = true;
                  ...
                  this.aByte3318 = buffer.readByte();
                  this.aByte3370 = buffer.readByte();
                  this.aByte3320 = buffer.readByte();
                  this.aByte3316 = buffer.readByte();
               } else if(opcode == 158) {
                  this.aByte3365 = 1;
               } else if(opcode == 159) {
                  this.aByte3365 = 0;
                  ...
                  for(index = 0; index < count; ++index) {
                     this.anIntArray3321[index] = buffer.readUnsignedShort();
                  }
               } else if(162 != opcode) {
                  if(opcode == 163) {
                     this.anInt3314 = buffer.readUnsignedByte(2021411147) * -429342161;
                  } else if(164 == opcode) {
                     this.anInt3301 = buffer.readUnsignedShort() * 1070155543;
                     this.anInt3369 = buffer.readUnsignedShort() * -454916931;
                  } else if(opcode == 165) {
                     ...
                     if(opcode == 179) {
                        ...
                        this.aClass437_3367.aFloat4856 = (float)buffer.readUnsignedSmart();
                        this.aClass437_3367.aFloat4858 = (float)buffer.readUnsignedSmart();
                        this.aClass437_3367.aFloat4852 = (float)buffer.readUnsignedSmart();
                        this.aClass437_3367.aFloat4853 = (float)buffer.readUnsignedSmart();
                        this.aClass437_3367.aFloat4854 = (float)buffer.readUnsignedSmart();
                        this.aClass437_3367.aFloat4855 = (float)buffer.readUnsignedSmart();
                     } else if(180 == opcode) {
                        this.anInt3344 = (buffer.readUnsignedByte(-962868852) & 255) * -591302333;
                     } else if(opcode == 181) {
                        this.aShort3348 = (short)buffer.readUnsignedShort();
                        ...
                        for(index = 0; index < count; ++index) {
                           ...
                           if(var11) {
                              ...
                              var8 = new Class526_Sub25(buffer.readInt());
                           }
            ...
            this.varbit = buffer.readUnsignedShort() * 641716999;
            if('\uffff' == this.varbit * -856216905) {
               this.varbit = -641716999;
            }

            this.varp = buffer.readUnsignedShort() * 1862704965;
            ...
            for(slot = 0; slot <= index; ++slot) {
               this.morphisms[slot] = buffer.readUnsignedShort();
