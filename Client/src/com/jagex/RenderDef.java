...
public class RenderDef implements Interface5, Interface12 {
	...
	void decode(RSByteBuffer buffer, int opcode, byte var3) {
		if (1 == opcode) {
			this.standAnimation = buffer.readBigSmart() * 1091988301;
			this.walkAnimation = buffer.readBigSmart() * 126041105;
		} else if (2 == opcode) {
			this.moveType1Anim = buffer.readBigSmart() * -2013971973;
		} else if (3 == opcode) {
			this.type1_180 = buffer.readBigSmart() * 1152456187;
		} else if (4 == opcode) {
			this.type1_90 = buffer.readBigSmart() * 298616021;
		} else if (opcode == 5) {
			this.type1_90_counter = buffer.readBigSmart() * -1335258299;
		} else if (opcode == 6) {
			this.runAnimation = buffer.readBigSmart() * 153856325;
		} else if (opcode == 7) {
			this.runRotate180Animation = buffer.readBigSmart() * -1855234777;
		} else if (8 == opcode) {
			this.runRotate90Animation = buffer.readBigSmart() * 897376457;
		} else if (opcode == 9) {
			this.runRotate90CounterAnimation = buffer.readBigSmart() * -556259403;
		} else if (26 == opcode) {
			...
			if (27 == opcode) {
				...
				for (var5 = 0; var5 < 6; ++var5) {
					this.anIntArrayArray7704[count][var5] = buffer.readShort();
				}
			} else if (28 == opcode) {
				...
				this.anInt7724 = buffer.readUnsignedShort() * 767373475;
			} else if (opcode == 31) {
				this.anInt7723 = buffer.readUnsignedByte(1330126189) * 641101329;
			} else if (32 == opcode) {
				this.anInt7700 = buffer.readUnsignedShort() * -1372069429;
			} else if (opcode == 33) {
				this.anInt7727 = buffer.readShort() * -54835271;
			} else if (34 == opcode) {
				this.anInt7720 = buffer.readUnsignedByte(2051671385) * -1953377087;
			} else if (opcode == 35) {
				this.anInt7729 = buffer.readUnsignedShort() * 142272027;
			} else if (36 == opcode) {
				this.anInt7730 = buffer.readShort() * -482929453;
			} else if (opcode == 37) {
				this.anInt7731 = buffer.readUnsignedByte(1276214664) * -219515623;
			} else if (opcode == 38) {
				this.walkUpwardsAnimation = buffer.readBigSmart() * -793107371;
			} else if (opcode == 39) {
				this.anInt7695 = buffer.readBigSmart() * 307802543;
			} else if (opcode == 40) {
				this.rotate180Animation = buffer.readBigSmart() * 1794388301;
			} else if (41 == opcode) {
				this.rotate90Animation = buffer.readBigSmart() * 1728954743;
			} else if (42 == opcode) {
				this.rotate90CounterAnimation = buffer.readBigSmart() * 2143181915;
			} else if (opcode == 43) {
				buffer.readUnsignedShort();
			} else if (opcode == 44) {
				buffer.readUnsignedShort();
			} else if (45 == opcode) {
				this.anInt7732 = buffer.readUnsignedShort() * 227354615;
			} else if (opcode == 46) {
				this.anInt7708 = buffer.readBigSmart() * 155609563;
			} else if (47 == opcode) {
				this.anInt7709 = buffer.readBigSmart() * 255757047;
			} else if (opcode == 48) {
				this.anInt7718 = buffer.readBigSmart() * -41625093;
			} else if (49 == opcode) {
				this.anInt7721 = buffer.readBigSmart() * -949711233;
			} else if (opcode == 50) {
				this.anInt7712 = buffer.readBigSmart() * 594618269;
			} else if (opcode == 51) {
				this.anInt7713 = buffer.readBigSmart() * -702683517;
			} else if (52 == opcode) {
				...
				for (var5 = 0; var5 < count; ++var5) {
					this.loopAnimations[var5] = buffer.readBigSmart();
				...
				this.anIntArray7699[count] = buffer.readUnsignedShort();
			} else if (56 == opcode) {
				...
				for (var5 = 0; var5 < 3; ++var5) {
					this.anIntArrayArray7698[count][var5] = buffer.readShort();
				}
			}
		}
